"""``monitoring-aiops secret`` — manage the encrypted credential store.

Secrets (Monitoring API keys) are stored in ``~/.monitoring-aiops/secrets.enc``
...
from monitoring_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: monitoring-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update MONITORING_AIOPS_MASTER_PASSWORD.[/]")
