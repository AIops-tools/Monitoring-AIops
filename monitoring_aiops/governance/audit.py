...
Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.monitoring-aiops/audit.db``.
...
_log = logging.getLogger("monitoring-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring MONITORING_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the MONITORING_AIOPS_HOME refactor
