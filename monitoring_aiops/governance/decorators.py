...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.monitoring-aiops/audit.db``
...
from monitoring_aiops.governance.audit import detect_agent, get_engine
from monitoring_aiops.governance.budget import BudgetExceeded, get_budget
from monitoring_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from monitoring_aiops.governance.patterns import PatternMatch, get_pattern_engine
from monitoring_aiops.governance.policy import PolicyResult, get_policy_engine
from monitoring_aiops.governance.readonly import is_read_only, read_only_denial
from monitoring_aiops.governance.sanitize import sanitize

_log = logging.getLogger("monitoring-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.monitoring-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("MONITORING_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("MONITORING_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (MONITORING_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set MONITORING_AUDIT_APPROVED_BY to the authorizing human (and "
            f"MONITORING_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`monitoring_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from monitoring_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``monitoring_aiops.ops.jobs`` → ``monitoring-aiops``
    ``mcp_server.server`` → ``monitoring-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("monitoring_aiops") or module.startswith("mcp_server"):
        return "monitoring-aiops"
    return "unknown"
