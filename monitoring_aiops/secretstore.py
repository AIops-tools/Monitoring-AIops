"""Encrypted secret storage for monitoring-aiops.

Secrets (Monitoring API keys) are NEVER written to disk in plaintext. They
live in ``~/.monitoring-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``MONITORING_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "monitoring-aiops"
CONFIG_DIR = Path.home() / ".monitoring-aiops"
MASTER_PASSWORD_ENV = "MONITORING_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``MONITORING_<TARGET>_PASSWORD``; ``prefix='MONITORING_'`` and
