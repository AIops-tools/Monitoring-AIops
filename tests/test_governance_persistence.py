...
throwaway home (``MONITORING_AIOPS_HOME``) and assert that the rows compliance
evidence is built from actually land on disk, and that a write runs and is
audited whether or not an approver annotation is set — the skill authorizes
nothing.
"""
...
from monitoring_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MONITORING_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="noc1")
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="noc1")
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("MONITORING_AUDIT_APPROVED_BY", "noc-alice")
    result = _drop_widget(name="w3", target="noc1")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "noc-alice"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: pause_sensor must record
    its inverse (resume_sensor) with the captured prior state on disk."""
    monkeypatch.setenv("MONITORING_AUDIT_APPROVED_BY", "noc-alice")
    conn = MagicMock(name="conn")
    conn.target.platform = "prtg"

    from mcp_server.tools import prtg_write as gov

    monkeypatch.setattr(gov, "_get_connection", lambda target=None: conn)
    result = gov.pause_sensor(object_id="2010", minutes=30, message="patching")
    assert result["priorState"] == {"paused": False}
    assert result.get("_undo_id")
    conn.prtg_post.assert_called_once()

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "resume_sensor"
    assert json.loads(undo[0]["undo_params"]) == {"object_id": "2010"}
    assert "priorState={'paused': False}" in undo[0]["note"]  # captured prior state

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["pause_sensor"]
