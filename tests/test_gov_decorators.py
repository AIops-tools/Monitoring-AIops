...
``monitoring_aiops.governance.*`` and binds state to an isolated home via
``MONITORING_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import monitoring_aiops.governance.budget as budget_mod
import monitoring_aiops.governance.decorators as dec_mod
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("MONITORING_MAX_TOOL_CALLS", "0")
