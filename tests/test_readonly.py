...
from monitoring_aiops.governance import READ_ONLY_ENV, is_read_only
from monitoring_aiops.governance.decorators import PolicyDenied, governed_tool
...
def test_env_var_is_this_tools_prefix():
    """Brand isolation: monitoring-aiops must not read another tool's switch."""
    assert READ_ONLY_ENV == "MONITORING_READ_ONLY"
...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can reach Orion/PRTG/Zabbix — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def remove_a_node(node_id: str = "42") -> dict:
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def list_nodes(target: str = "lab") -> list:
        return [{"caption": "core-sw1"}]

    assert list_nodes() == [{"caption": "core-sw1"}]


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. alert_acknowledge) must be refused too."""

    @governed_tool(risk_level="medium")
    def acknowledge_an_alert(alert_id: str = "42") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "remove_node" in dropped and "unmanage_node" in dropped
        # Acknowledging and muting are triage, but they are still writes.
        assert "alert_acknowledge" in dropped and "mute_alerts" in dropped
        assert "active_alerts" not in dropped and "swql_query" not in dropped
        ...
        assert "active_alerts" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
