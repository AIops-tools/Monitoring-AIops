...
> perpetual 100-sensor Freeware edition (with the API) is the easiest live
> check; SolarWinds is a 30-day trial only — mock-only past that, the largest
> verification debt.**
...
The wizard asks, per target, for the **platform** (`solarwinds` / `prtg`), the
**host**, the **port** (defaults 17778 for SolarWinds, 443 for PRTG), the
**Orion username** (SolarWinds only), and the **secret** — the Orion account
password or the PRTG API token. Non-secret connection details go to
`~/.monitoring-aiops/config.yaml`; the secret is stored **encrypted** into
`~/.monitoring-aiops/secrets.enc`. Example config (one config can span both
NOCs):
...
- The secret (Orion password / PRTG API token) is **never** written to disk in
...
- The secret is used for HTTP Basic auth (SolarWinds) or as the PRTG API token at
  request time and held only in memory; it is never logged or echoed. Exception
...
- **High-risk** ops (`unmanage_node`, `remove_node`) require an approver — set
...
- **Time-boxed** ops require an end time / duration: `mute_alerts`,
  `schedule_maintenance` (SolarWinds), and `schedule_maintenance_prtg` (PRTG,
  in minutes). This prevents forgotten, indefinite suppression windows.
