...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/monitoring-aiops` to
  `@zw008/monitoring-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/monitoring-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/monitoring-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install monitoring-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/monitoring-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [monitoring-aiops, uvx]` — either one suffices — with every variable kept
...
- **A failed host lookup says so.** Swallowing it made every `host` `null`, which is indistinguishable from "these problems have no host" — a failure wearing the shape of an absence (bug class #3). The reason is now surfaced as `hostLookupError` on both the problem list and the alert payload.
