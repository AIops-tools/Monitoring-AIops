...
the SAME governance path as any other call (audited and budget-checked, its
``risk_level`` carried into the audit row like any direct call). ``undo_apply``
is itself governed.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so it is audited and its
    risk_level recorded there. Pass dry_run=True to preview the inverse call
